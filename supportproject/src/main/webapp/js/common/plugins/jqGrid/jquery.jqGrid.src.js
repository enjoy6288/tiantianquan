...
//module begin
    ...
    $.jgrid.extend({
        ...
        hideCol: function (getLocalRowcolname) {
            return this.each(function () {
                ...
                if (cm.editable === true && ed === true && !cc.hasClass("not-editable-cell") && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t, nm, iRow, iCol))) {
                    ...
                    $("input, select, textarea", cc).bind("keydown", function (e) {
                        ...
                        if (e.keyCode === 13 && !e.shiftKey) {
                            ...
                            //debugger;
                            if("function" == typeof $t.p.colModel[iCol].enterCallback){
            ...
            return this.each(function () {
                ...
                if ($t.grid && $t.p.cellEdit === true) {
                    ...
                    if (i === $t.p.colModel.length) {
                        if (iRow = Number(iRow) + 1, 0 === $($t).find("tbody tr").eq(iRow).length) {
                            if ($t.p.triggerAdd === false) {
                                return iRow -= 1, void $($t).jqGrid("saveCell", iRow, iCol)
                            }
                            if($($t).attr("id")!== "fixedGrid" && THISPAGE.newId==null){
                    ...
                    nCol !== false ? $($t).jqGrid("editCell", iRow, nCol, true) : $t.p.savedRow.length > 0 && $($t).jqGrid("saveCell", iRow, iCol)
                }
                /*if (!$t.grid || $t.p.cellEdit !== true) {
                 return;
                 }
