 ...
 * function test() {
 * 
 * var msg = "abcdefgh"; var bt = strToBt(msg);
 * 
 * var key = "12345678"; var keyB = strToBt(key);
 * 
 * var encByte = enc(bt,keyB);
 * 
 * var enchex = bt64ToHex(encByte); endata.value=enchex;
 * 
 * var encStr = hexToBt64(enchex); alert("encStr="+encStr); var eByte = new
 * Array(); for(m=0;m<encStr.length;m++){ eByte[m] =
 * parseInt(encStr.substring(m,m+1)); } var decbyte= dec(eByte,keyB) var decmsg=
 * byteToString(decbyte); alert("decbyte="+decbyte); alert("decmsg="+decmsg); }
 */